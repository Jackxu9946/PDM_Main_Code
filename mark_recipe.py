...
def mark_recipe(user_id, recipe_id):
    cur.execute("SELECT * FROM recipe_manager.recipes WHERE recipe_id = %d")
    exist = cur.fetchone()
    if not exist:
        ...
        cur.execute("SELECT ingredient FROM recipe_manager.ingredient_to_recipe WHERE recipe_id = %s", (recipe_id,))
        ...
        for i in ingredients:
            ...
            cur.execute("SELECT current_quantity FROM recipe_manager.ingredient_to_recipe WHERE recipe_id = %s"
                        "AND ingredient_id = %s ", (recipe_id, i))
            quantity_need = cur.fetchone()

            # get the quantity of the needed ingredient in the pantry
            cur.execute("SELECT current_quantity FROM recipe_manager.pantry WHERE user_id = %s "
                        "AND ingredient_id = %s ", (recipe_id, i))
            ...
            if quantity_remained >= 0:
                ...
                cur.execute("UPDATE recipe_manager.pantry SET current_quantity = %d "
                            "WHERE user_id = %d AND ingredient_id = %d", (quantity_remained, user_id, i))
            # not enough quantity for this ingredient
            """
            cur.execute("SELECT * FROM recipe_manager.pantry WHERE user_id = %s and ingredient_id = %s ", (user_id, i))
            result = cur.fetchall()
            if not result:
...
def rate_recipe(user_id, recipe_id):
    ...
    cur.execute("INSERT INTO recipe_manager.rating(user_id, recipe_id, rating, rating_date, review_text ) "
...
def update_item(user_id, ingredient_id, quantity_updated):
    cur.execute("UPDATE recipe_manager.pantry SET current_quantity = %s "
...
def add_item(user_id, ingredient_id):
    current_quantity = 1
    cur.execute("INSERT INTO recipe_manager.pantry(user_id, ingredient_id, current_quantity)"
